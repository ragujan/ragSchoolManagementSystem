...
class StudentQuery extends DBh
{
    ...
    public function insertstudent($fname, $lname, $email, $age, $gender)
    {
        ...
        if ($emailCheck) {
            ...
            $state = false;
            $status = 0;
            $query = "INSERT INTO `student` (`student_fname`,`student_lname`,`student_email`,`student_age`,`student_gender`,`student_password`,`student_status`)
            VALUES (?,?,?,?,?,?,?)";
            $statement = $this->connect()->prepare($query);
